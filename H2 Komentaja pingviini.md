...
Kätevä toiminto/komento jonka löysin on _-i_ (ignore case) joka ohittaa "case sensitive" toiminnon. Komentorivihän on muutoin totaalisen tarkka isoista ja pienistä kirjaimista. Tällä komennolla etsin tekstiä "debian" aiemmin esitellystä /etc kansiosta. Komentona käytettiin _grep -i "debian" /etc/os-release_.
